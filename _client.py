...
def create_user_card(parent, user):
    ...
    display_name = 'You' if user == _username else user
    CTkLabel(card, text=display_name, 
            font=("Arial", 14, "bold"),
            text_color="white").place(x=60, y=20)
...
def refresh_user_cards(user_list):
    ...
    # Filter out empty and None values, and remove duplicates
    filtered_users = []
    seen_users = set()
    for u in user_list:
        if u and u.strip() and u not in seen_users:
...
def add_users(user_list):
    ...
    rm_dup.clear()
    for u in user_list:
        if u and u.strip():  # Filter out empty strings
            rm_dup.add(u)
    ...
    try:
        ...
        while True:
            ...
            if get_o_users.startswith('OU:'):
                ...
                # Filter out empty strings and clean the user list
                cleaned_users = [u.strip() for u in user if u.strip()]
                #Remove duplicate user for iteration
                rm_dup_u = add_users(cleaned_users)
