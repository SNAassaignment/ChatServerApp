...
def remove_users(user):
    global users
    # Remove from users list (dicts with 'name')
    users = [u for u in users if u['name'] != user]

    # Remove from showed_users set (if present)
    if user in showed_users:
        showed_users.remove(user)

    # Refresh UI
    current_users = set(u['name'] for u in users)
    refresh_user_cards(current_users)
    ...
    try:
        ...
        while True:
            ...
            if get_o_users.startswith('LEFT-'):
                lft_user = get_o_users.split('-')[1].strip()
